...
struct Config
    ...
    # snapshots for trait distributions (histograms/heatmaps)
    snapshot_start::Int
    snapshot_every::Int
    sample_size::Int
...
# action codes: 0 = avoid, 1 = war, 2 = trade
# ============================================================
...
function trade_minimal_law!(st::State, j::Int, k::Int)
    ...
    # complementary comparative advantage cases
    ...
    new_R2_2 = receive2 == :r2 ? (R2_2 + g1_gives) : (R2_2 - g2_gives)  # (fixed)

    # Optional safeguard: only accept if both improve min-resource
...
function trade_cobb_douglas!(st::State, j::Int, k::Int)
    # Your commented block turned into a function.
    # NOTE: this assumes r1,r2 positive (which they are after +min_resource).
...
function resolve_pair!(st::State, cfg::Config, j::Int, k::Int, min_resource::Float64)
    ...
    if (aj == 1) && (ak == 1)
        # war-war: winner takes all
        winner, loser = rand(Bool) ? (j, k) : (k, j)
        st.r1[winner] += st.r1[loser]
        st.r2[winner] += st.r2[loser]
        st.r1[loser]  = -min_resource
        st.r2[loser]  = -min_resource
        war = 1.0

    elseif (aj == 1) && (ak != 1)
        # raid j -> k
        if (cfg.avoidance == :constrained) && (ak != 2)
            # constrained: raider can only exploit if target tried to trade (ak==2)
            avoid = 1.0
        else
    ...
    elseif (ak == 1) && (aj != 1)
        # raid k -> j
        if (cfg.avoidance == :constrained) && (aj != 2)
            avoid = 1.0
        else
